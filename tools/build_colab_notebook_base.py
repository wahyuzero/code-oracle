...
cells_raw = [
    ...
    },
    {
        ...
        "content": [
            ...
            "",
            "# 1. Save Base weights in BF16 (~312 MB)",
            "import torch",
            "bf16_state_dict = {k: v.to(torch.bfloat16) if v.is_floating_point() else v for k, v in agent.model.state_dict().items()}",
            'save_file(bf16_state_dict, str(out_dir / "model.safetensors"))',
            'bf16_size_mb = (out_dir / "model.safetensors").stat().st_size / (1024 * 1024)',
            'print(f"    [+] Saved base neural weights (BF16): {bf16_size_mb:.2f} MB")',
            "",
